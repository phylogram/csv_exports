...
class ExportCSV {
  ...
  public function __construct($start, $stop, string $mode, $exclude) {
    ...
    $this->blacklist = new \Drupal\phylogram_datatransfer\import_model\Blacklist();
  }
  ...
  public function execute() {
    ...
    foreach ($settings as $topic => $setting) {
      ...
      foreach ($time_frames as $time_frame) {
        ...
        $folder_names = \Drupal\phylogram_datatransfer\export_model\FolderNaming::translateTime($setting['folder_structure'], $setting['file_name'], $setting['parse_file_name'], $this_time->getTimestamp());
        ...
        foreach ($database_topic_query->fetchRow() as $row) {
          ...
          foreach ($row as $column) {
            $exclude = $this->blacklist->contains([$column]);
            if ($exclude) {
              continue;
            }
          }
