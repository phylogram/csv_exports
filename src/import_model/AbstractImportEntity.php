...
abstract class AbstractImportEntity extends \Drupal\phylogram_datatransfer\import_model\AbstractImport {
  ...
  /**
   * Overwrites parent with foreach instead of while and cleans drupal cache in
   * loop
   *
  ...
  protected function _modifyRow() {
    $this->row = $this->sortNestedFields($this->row, $this->sort_fields);
  }
  ...
  protected function _createFields(array $fields) {
    ...
    foreach ($this->fields as $aliases) {
      $this->sort_fields[$aliases['export_name']] = $aliases['import_name'];
    }
  }
